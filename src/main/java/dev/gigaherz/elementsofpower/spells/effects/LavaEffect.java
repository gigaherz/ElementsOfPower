...
public class LavaEffect extends SpellEffect
{
    ...
    {
        return cast.world.dimensionType().ultraWarm() ? +1 : 0;
    }
    ...
    public void processBlockWithinRadius(InitializedSpellcast cast, BlockPos blockPos, BlockState currentState, float r, @Nullable HitResult mop)
    {
        ...
        if (currentState.isAir())
        {
            ...
            {
                cast.world.setBlockAndUpdate(blockPos, Fluids.LAVA.defaultFluidState().createLegacyBlock());
            }
            else
            {
                cast.world.setBlockAndUpdate(blockPos, Fluids.FLOWING_LAVA.defaultFluidState().setValue(LavaFluid.LEVEL, 8).createLegacyBlock());
            }
        }
    }
}
