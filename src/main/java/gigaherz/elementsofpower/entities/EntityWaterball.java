...
public class EntityWaterball extends EntityThrowable implements IVariableSize {
    ...
    protected void onImpact(MovingObjectPosition p_70184_1_)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            this.worldObj.spawnParticle(EnumParticleTypes.WATER_SPLASH, this.posX, this.posY, this.posZ, 1.0D, 0.0D, 0.0D);
        }
            ...
            for (int z = pz - damageForce; z <= pz + damageForce; z++) {
                for (int x = px - damageForce; x <= px + damageForce; x++) {
                    for (int y = py - damageForce; y <= py + damageForce; y++) {
                        ...
                        if (block == Blocks.air) {
                            worldObj.setBlockState(np, Blocks.flowing_water.getDefaultState().withProperty(BlockDynamicLiquid.LEVEL, 15), 2);
                        }
                    }
                }
            }
