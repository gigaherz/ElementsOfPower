...
public class Waterball extends BallBase
{
    boolean spawnSourceBlocks = false;

    public Waterball(SpellBall parent)
    {
        super(parent);
    }
    ...
    protected void processBlockWithinRadius(BlockPos blockPos, IBlockState currentState, int layers)
    {
        ...
        if (block == Blocks.air)
        {
            ...
            {
                world.setBlockState(blockPos, Blocks.flowing_water.getDefaultState().withProperty(BlockDynamicLiquid.LEVEL, 0));
            }
            else
            {
                world.setBlockState(blockPos, Blocks.flowing_water.getDefaultState().withProperty(BlockDynamicLiquid.LEVEL, 15));
            }
        }
    }
