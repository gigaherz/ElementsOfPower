...
public class Cushion extends BallBase
{
    public Cushion(SpellBall parent)
    {
        super(parent);
    }
    ...
    protected void processBlockWithinRadius(BlockPos blockPos, IBlockState currentState, int layers)
    {
        ...
        if (block == Blocks.air)
        {
            world.setBlockState(blockPos, ElementsOfPower.cushion.getDefaultState().withProperty(BlockCushion.DENSITY, 16));
        }
    }
